...
public class Kernel
{
	...
	case INTERRUPT_SOFTWARE: // System calls
	    ...
	    case WAIT:
			...
			// check to make sure my TCB is not null
			if( null != (myTcb = scheduler.getMyTcb()) )
			{
				// get the tid of the scheduler tcb
				int myTcbTid = myTcb.getTid();
				
				// return status of enqueuAndSleep with myTcbTid
				return waitQueue.enqueueAndSleep(myTcbTid);
			}
	    ...
	    case EXIT:
			...
			if( null != (myTcb = scheduler.getMyTcb()) )
			{
				...
				// get the pid of the schediler tcb
				int myTcbPid = myTcb.getPid();
				
				if ( myTcbPid > -1 )
				{
					// dequeue thread
					waitQueue.dequeueAndWakeup(myTcbPid, myTcbTid);
					
					// delete thread from scheduler that is exiting
